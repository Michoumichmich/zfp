...
namespace syclZFP {
    ...
    void copy_length_launch(sycl::queue &q, ushort *bitlengths, size_t *chunk_offsets, size_t first, int nstreams_chunk) {
        ...
        q.parallel_for(kernel_range, [=](sycl::nd_item<1> item) {
    ...
    inline void load_to_shared(sycl::nd_item<2> item, const uint *streams,                   // Input data
    ...
    void concat_bitstreams_chunk(sycl::nd_item<2> item, uint *__restrict__ streams,
    ...
    void chunk_process_launch(sycl::queue &q, uint *streams,
        ...
        if (nbitsmax <= 352) {
            ...
            cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_blocks, concat_bitstreams_chunk<tile_size, num_tiles>, tile_size * num_tiles, shmem);
            max_blocks *= num_sm;
            cod
                    max_blocks = std::min(nstream_chunk, max_blocks);
            sycl::range<2> threads(num_tiles, tile_size);
            cudaLaunchCooperativeKernel((void *) concat_bitstreams_chunk<tile_size, num_tiles>, sycl::range<2>(1, max_blocks), threads, kernelArgs, shmem, 0);
            ...
            cudaLaunchCooperativeKernel((void *) concat_bitstreams_chunk<tile_size, num_tiles>,
                                        sycl::range<2>(1, max_blocks), threads, kernelArgs, shmem, 0);
