...
#define PREPEND_SYCL(x) SYCL_ ## x
#define DESCRIPTOR_INTERMEDIATE(x) PREPEND_SYCL(x)
#define DESCRIPTOR DESCRIPTOR_INTERMEDIATE(DIM_INT_STR)

#include "zfpEndtoendBase.c"

// sycl entry functions
static void
...
static int
setupSYCLConfig(void **state, stride_config stride)
{
  int result = initZfpStreamAndField(state, stride);

  struct setupVars *bundle = *state;
  assert_int_equal(zfp_stream_set_execution(bundle->stream, zfp_exec_sycl), 1);
...
{
  return setupSYCLConfig(state, PERMUTED);
}

static int
setupInterleaved(void **state)
{
  return setupSYCLConfig(state, INTERLEAVED);
}

static int
setupReversed(void **state)
{
  return setupSYCLConfig(state, REVERSED);
}

static int
setupDefaultStride(void **state)
{
  return setupSYCLConfig(state, AS_IS);
}
