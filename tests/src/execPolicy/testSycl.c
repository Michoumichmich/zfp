...
setup(void **state) {
    ...
    /* set SYCL policy */
    assert_int_equal(1, zfp_stream_set_execution(bundle->stream, zfp_exec_sycl));
...
static void
given_withSYCL_when_4dCompressSYCLPolicy_expect_noop(void **state) {
    struct setupVars *bundle = *state;
...
static void
given_withSYCL_when_4dDecompressSYCLPolicy_expect_noop(void **state) {
    struct setupVars *bundle = *state;
...
int main() {
    const struct CMUnitTest tests[] = {
            cmocka_unit_test_setup_teardown(given_withSYCL_when_4dCompressSYCLPolicy_expect_noop, setup, teardown),
            cmocka_unit_test_setup_teardown(given_withSYCL_when_4dDecompressSYCLPolicy_expect_noop, setup, teardown),
    };
